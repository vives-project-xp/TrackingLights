...
void Adafruit_NeoPixel::updateLength(uint16_t n) {
  ...
  numBytes = (n * ((wOffset == rOffset) ? 3 : 4)) + 2;
...
#define CYCLES_800      (F_CPU /  94000) //working: 96000 - 1100000
#define CYCLES_400_T0H  (F_CPU / 2000000)
#define CYCLES_400_T1H  (F_CPU /  833333)
#define CYCLES_400      (F_CPU /  400000)
    
  /*  #define CYCLES_800_T0H  (F_CPU / 4000000) //Original timings
 ...
 */
    ...
    uint8_t        *p   = pixels, *end = p + numBytes, pix, mask; //colour data variables and pointers
    volatile uint8_t *set = portSetRegister(pin), *clr = portClearRegister(pin);
    uint32_t        cyc; //previous cyc count
...
#define CYCLES_800_T0H  (F_CPU / 333333333)
#define CYCLES_800_T1H  (F_CPU / 166666666)
#define CYCLES_800      (F_CPU /  120000000)//1uS
#define CYCLES_400_T0H  (F_CPU / 2000000)
#define CYCLES_400_T1H  (F_CPU /  833333)
#define CYCLES_400      (F_CPU /  400000)
...
*/
//D1, D2, D3...Dn
void Adafruit_NeoPixel::setPixelColor(uint32_t Ic1, uint32_t Ic2, uint16_t n, uint32_t c) {
    uint8_t *p,
 ...
 //C1 and C2
 void Adafruit_NeoPixel::setPixelCurrent(uint32_t Ic) {
         uint8_t *p,
         Ir = (uint8_t)(Ic >> 16),
         Ig = (uint8_t)(Ic >>  8),
         Ib = (uint8_t)Ic;
         if(wOffset == rOffset) {
             ...
             p = &pixels[4];
             uint8_t Iw = (uint8_t)(Ic >> 24);
             p[wOffset] = brightness ? ((Iw * brightness) >> 8) : Iw;
         }
         p[rOffset] = Ir;
         p[gOffset] = Ig;
         p[bOffset] = Ib;
...
*/
void Adafruit_NeoPixel::fill(uint32_t Ic1, uint32_t Ic2, uint32_t c, uint16_t first, uint16_t count) {
  uint16_t i, end;
  ...
  for(i = first; i < end; i++) {
    this->setPixelColor(Ic1, Ic2, i, c);
  }
}
